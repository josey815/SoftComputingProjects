...
namespace R10546040FuzzySetNamespace
{
    ...
    public class Universe
    {
        ...
        public void Save(StreamWriter sw)
        {
            ...
            sw.WriteLine($"Minimum:{lowerBound}");
            sw.WriteLine($"Maximum:{upperBound}");
            sw.WriteLine($"Resolution:{increment}");
        }
        ...
        public void Open(StreamReader sr)
        {
            ...
            Title = items[1];
            items = sr.ReadLine().Split(':');
            LowerBound = Convert.ToDouble(items[1]);
            items = sr.ReadLine().Split(':');
            UpperBound = Convert.ToDouble(items[1]);
            items = sr.ReadLine().Split(':');
            Increment = Convert.ToDouble(items[1]);
        }
    }
}
